...
try:
    from astronomist_agents.llm_config import get_celeste_provider_and_model
except ImportError:
    from llm_config import get_celeste_provider_and_model


def _create_celeste_client():
    """Build a Celeste client for Groq (default), Gemini, or OpenAI."""
    provider, model_id = get_celeste_provider_and_model()
    try:
        from celeste import Capability, create_client
        ...
        from celeste import Modality, Operation, create_client
...
🎯 TASK: Analyze these observational characteristics for physical plausibility and exoplanet indicators."""
        ...
        output = await get_client().generate(prompt=prompt)
