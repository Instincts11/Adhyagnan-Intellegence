...
from astronomist_agents.johannes_kepler_agent import create_agent, Runner, lookup_exoplanet_archive
...
APP_TITLE = "Adhyagnan Agents & ML API"
APP_VERSION = "1.0.0"
...
@app.get("/")
async def root():
    return {"message": "Adhyagnan Agents & ML API", "status": "active"}
...
class PlanetLookupResponse(BaseModel):
    success: bool
    data: List[Dict[str, Any]] = []
    error: Optional[str] = None
    source: str = "astroquery"


@app.get("/kepler/planet", response_model=PlanetLookupResponse)
async def lookup_planet(name: str):
    """Return NASA archive parameters for one confirmed world via astroquery."""
    try:
        result = await asyncio.to_thread(lookup_exoplanet_archive, name)
        rows = result.get("results") or []
        if result.get("success") and rows:
            return PlanetLookupResponse(success=True, data=rows, source="astroquery")
        return PlanetLookupResponse(
            success=False,
            data=[],
            error=result.get("message") or f'No exoplanet found for "{name}"',
            source="astroquery",
        )
    except Exception as e:
        logger.exception("Planet lookup failed for %s", name)
        return PlanetLookupResponse(success=False, data=[], error=str(e), source="astroquery")
